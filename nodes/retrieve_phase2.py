...
def retrieve_phase2_node(state: AgentState) -> AgentState:
    ...
    if multi_query_enabled:
        vector_top_k = int(AppConfig.PHASE2_PER_QUERY_VECTOR_TOP_K)
        keyword_top_k = int(AppConfig.PHASE2_PER_QUERY_KEYWORD_TOP_K)
        keep_top_m = int(AppConfig.PHASE2_PER_QUERY_KEEP_TOP_M)
        workers = max(
            1,
        ...
        phase2_candidates = _aggregate_multi_query_results(query_runs)
    else:
        ...
        phase2_candidates = [_result_to_candidate(r) for r in single_run["merged"]]

    phase2_query_routes: list[dict] = []
    if multi_query_enabled:
        phase2_query_routes = _build_query_route_records(
            query_runs=query_runs,
            per_route_top_k=int(AppConfig.RETRIEVAL_MULTI_QUERY_ROUTE_TOP_K),
        )

    state["phase2_candidates"] = phase2_candidates
    state["phase2_query_routes"] = phase2_query_routes
    # Keep downstream compatibility: existing nodes read phase1_query_routes.
    state["phase1_query_routes"] = phase2_query_routes
        ...
        StepLog(
            ...
            info={
                ...
                "memory": {
                    ...
                    "query_route_count": len(phase2_query_routes),
