...
def _has_memory_signal (state :AgentState )->bool :
    ...
    if short_term_memory and short_term_memory not in {"none","No relevant short-term memory","None"}:
        return True 
...
def _apply_memory_fallback (state :AgentState )->None :
    state ["draft_answer"]="Insufficient memory evidence, external documentation needs to be retrieved."
...
def compose_memory_draft_node (state :AgentState )->AgentState :
    ...
    if shortcut_no_memory :
        ...
        for attempt in range (1 ,total_attempts +1 ):
            ...
            # Memory draft failure should not interrupt the main process and directly downgrade to the retrieval path
                llm_error =f"{type(e).__name__}: {e}"
                used_fallback =True 
                break 
