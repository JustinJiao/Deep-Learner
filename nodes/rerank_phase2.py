...
def rerank_phase2_node (state :AgentState )->AgentState :
    ...
    phase2_query_routes =state .get ("phase2_query_routes",[])or state .get ("phase1_query_routes",[])or []
    ...
    if multi_query_enabled and len (phase2_query_routes )>1 :
        for route in phase2_query_routes :
        ...
        state ["phase2_query_routes"]=compact_routes 
        state ["phase1_query_routes"]=compact_routes 
    else :
        state ["phase2_query_routes"]=[]
    ...
    # Release phase2 to retrieve intermediate results and reduce subsequent state copy costs.
