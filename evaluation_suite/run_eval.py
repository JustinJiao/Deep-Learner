...
# 1) json/jsonl file
# 2) HuggingFace save_to_disk directory (including state.json)
...
def _extract_retrieval (state :dict [str ,Any ])->tuple [list [dict [str ,Any ]],list [str ]]:
    ...
    for rank ,item in enumerate (context_pool ,start =1 ):
        ...
        # Remove duplicates in order of first appearance and keep the highest score
    ...
    try :
        ...
        if fast_executor :
            ...
            def _fast_planner_node (state :dict [str ,Any ])->dict [str ,Any ]:
                ...
                # The READS field of ComposePrompt is explicitly completed in the fast path to avoid PromptContractError
                state .setdefault ("rewritten_query",state .get ("query",""))
                state .setdefault ("long_term_memory","No associated long-term memory")
...
# When hard timeout is not enabled, it is executed directly in the current process to avoid the process startup overhead of each query.
...
def _aggregate_llm_node_metrics (records :list [dict [str ,Any ]])->dict [str ,Any ]:
    ...
    for node in sorted (node_calls .keys ()):
        ...
        # Note: The following quality indicators are "the final answer quality proxy for this node's execution sample" and are used for horizontal comparison.
